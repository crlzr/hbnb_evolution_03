...
class Country(Base):
    ...
    @staticmethod
    def places(country_code = "", amenities = "", max_price = 500, min_price = 0):
        ...
        if max_price != "500":
            query_txt = query_txt + where_and + "pl.price_per_night <= " + str(max_price)
            where_and = " AND "

        if min_price != "0":
            query_txt = query_txt + where_and + "pl.price_per_night >= " + str(min_price)
